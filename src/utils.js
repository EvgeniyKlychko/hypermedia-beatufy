...
export function segmentFaceContour(
  face_img_tensor,
  face_img_tensor_small,
  keypoints_all,
  blur_kernel_size
) {
  //console.log()
...
export function segmentFace(
  ...
  var mask = tf.zeros([face_img_tensor_small.shape[0], face_img_tensor_small.shape[1], 1]);
...
function sample_avg_color(img, color_coords, square_side_px) {
  const sample = img.slice(
    [parseInt(color_coords[0]), parseInt(color_coords[1])],
...
function keypoints_to_pts(keypoints_specific, keypoints_all) {
  ...
  for (var i = 0; i < keypoints_specific.length; i++) {
    square_point_data.push(
      keypoints_all[keypoints_specific[i]][0],
...
function boxBlur(mask, kernel_size) {
  ...
  const kernel = tf
    .ones([kernel_size, kernel_size, mask.shape[2], mask.shape[2]])
...
export function increase_brightnessv3(rgb_img, value) {
  const rChannel = rgb_img.slice([0, 0, 0], [-1, -1, 1]).add(
    tf
      .tensor1d([1])
      .sub(rgb_img.slice([0, 0, 0], [-1, -1, 1]))
      .mul(value)
  );
  const gChannel = rgb_img.slice([0, 0, 1], [-1, -1, 1]).add(
    tf
      .tensor1d([1])
      .sub(rgb_img.slice([0, 0, 1], [-1, -1, 1]))
      .mul(value)
  );
  const bChannel = rgb_img.slice([0, 0, 2], [-1, -1, 1]).add(
    tf
      .tensor1d([1])
      .sub(rgb_img.slice([0, 0, 2], [-1, -1, 1]))
      .mul(value)
  );
...
export function lowPassHighPass(image, high_pass_threshold, blur_sigma, truncation_thresh_kernel) {
  ...
  const gaussian_kernel = tf.concat(
    [gaussian_kernel_single_ch, gaussian_kernel_single_ch, gaussian_kernel_single_ch],
    2
  );
...
const canvasName = 'help';

export const bilateral_filtering = (diameter, sigma) => {
  let src = cv.imread(canvasName);
  let dst = new cv.Mat();
...
export const bilateral_filter_gauss = (diameter, sigma, kernelSize) => {
  ...
  let smooth_rate_mat = new cv.Mat(src.rows, src.cols, cv.CV_32FC3, [
    smooth_rate,
    smooth_rate,
    smooth_rate,
    smooth_rate,
  ]);
  let smooth_rate_rev_mat = new cv.Mat(src.rows, src.cols, cv.CV_32FC3, [
    1 - smooth_rate,
    1 - smooth_rate,
    1 - smooth_rate,
    1 - smooth_rate,
  ...
  cv.GaussianBlur(
    dst_1,
    dst_2,
    { width: gauss_kernel_size, height: gauss_kernel_size },
    0,
    0,
    cv.BORDER_DEFAULT
  );
...
export const bilateral_filter_mix = (diameter, sigma) => {
  let smooth_rate = 0.7;
  ...
  src.convertTo(src, cv.CV_32FC3);
  dst_1.convertTo(dst_1, cv.CV_32FC3);

  // original equation
  // im_smooth = np.minimum(smooth_rate * im_ga + (1 - smooth_rate) * im_bgr, 255).astype('uint8')

  cv.multiply(smooth_rate_mat, dst_1, mul_1);
  // console.log('running')
