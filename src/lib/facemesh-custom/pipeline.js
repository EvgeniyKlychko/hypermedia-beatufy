'use strict';
import {CROP_SIZE} from "../../constants/crop-size";
...
class Pipeline {
  ...
  transformRawCoords(rawCoords, box, angle, rotationMatrix) {
    ...
    const boxCenter = [
      ...box_1.getBoxCenter({ startPoint: box.startPoint, endPoint: box.endPoint }),
      1,
    ];
    ...
    return tf.tidy(() => {
      return this.regionsOfInterest.map((box, i) => {
        ...
        if (boxLandmarksFromMeshModel) {
          ...
          angle = util_1.computeRotation(
            box.landmarks[indexOfNose],
            box.landmarks[indexOfForehead]
          );
        } else {
        ...
        const faceCenter = box_1.getBoxCenter({
          startPoint: box.startPoint,
          endPoint: box.endPoint,
        });
        const faceCenterNormalized = [
          faceCenter[0] / input.shape[2],
          faceCenter[1] / input.shape[1],
        ];
        ...
        const face = box_1
          .cutBoxFromImageAndResize(boxCPU, rotatedImage, [this.meshHeight, this.meshWidth])
          .div(255);
        ...
        this.regionsOfInterest[i] = Object.assign({}, landmarksBox, {
  ...
  async getFace(input) {
    ...
    return tf.tidy(() => {
      return this.regionsOfInterest.map((box, i) => {
        ...
        const faceNormal = box_1.cutBoxFromImageAndResize(boxCPU, input, CROP_SIZE).div(255);
        const [, flag, coords] = this.meshDetector.predict(face);
        const coordsReshaped = tf.reshape(coords, [-1, 3]);
        const rawCoords = coordsReshaped.arraySync();
        const transformedCoordsData = this.transformRawCoords(
